...
def _bulk_preflight_filter_devices(
    ...
    preflight_enabled = _env_bool("BULK_PREFLIGHT_ENABLED", "1")
    ...
    skip_unready = _env_bool("BULK_PREFLIGHT_SKIP_UNREADY", "1")
    strict_require_ready = _env_bool("BULK_PREFLIGHT_REQUIRE_READY", "0")
    ...
    for device in devices or []:
        ...
        if not include_unvalidated:
            if ready_recent:
                should_skip = False
            elif classification in {"no_ping", "ping_ok_login_fail"} and skip_unready:
                should_skip = True
            elif strict_require_ready and skip_unready:
                should_skip = True
...
def run_backup_all(tenant_slug):
    ...
    for device in devices:
        ...
        if single_group_mode:
            # Em contexto de grupo/subgrupo, enfileira por dispositivo (sem colapsar em 1 task de grupo),
            # para refletir corretamente o volume no modal e melhorar paralelismo percebido.
    ...
    if single_group_mode:
        ...
        for device in direct_no_group_devices:
            if _backup_queue_for_device(device) == "vpn_queue":
                queued_vpn_devices += 1
            if device.group and uses_jump_host(device.group, device=device):
                queued_jump += 1
        if is_ajax:
            append_task_log(
                ...
                "Backup em massa",
                (
                    "Modo de grupo/subgrupo detectado: enfileiramento por dispositivo ativado "
                    f"({queued_direct} task(s) para {len(direct_no_group_devices)} dispositivo(s))."
                ),
                "info",
            )
    ...
    # Fase 2: jump host (por grupo no bulk global)
    ...
    if single_group_mode:
            ...
            if is_ajax:
                append_task_log(
                    ...
                    (
                        ...
                        f"Fase 2 (jump host): {queued_jump_groups} grupo(s). "
        ...
        if is_ajax:
            append_task_log(
                ...
                "Backup em massa",
                (
                    f"Sem fase direta. Iniciando fase 2 (jump host: {queued_jump_groups} grupo(s)) "
    ...
    if is_ajax:
        ...
        total_tasks = len(child_task_ids) + (queued_jump_groups + queued_vpn_groups if phased_deferred else 0)
        update_task_meta(
            bulk_task_id,
            is_bulk=True,
        ...
        if phased_deferred:
            append_task_log(
                ...
                (
                    ...
                    f"Fase 2: {queued_jump_groups} grupo(s) jump host. "
